...
7. **行数与 `TOP`（单次最多 1000 条明细）**：除 `COUNT(*)`、聚合标量、标量子查询、占位 `SELECT 1`、以及仅用于列探测的极小样本（如 `TOP (10)` / `TOP (100)` 且语义为探测结构）外，凡可能返回多行明细的 `SELECT`（含子查询中作为派生表的多行结果），**单次查询返回行数不得超过 1000 条**（**主表+明细 JOIN 时见第 7a 条**）。  
   - 用户**未**在【用户问题】中给出条数上限时：**单表**明细使用 **`SELECT TOP (1000) …`**（`TOP (1000)` 紧接在 `SELECT` 之后），不得以「未要求 TOP」为由写出无上限明细结果集。  
   - 用户**明确**给出上限 `n` 时：若 `n ≤ 1000`，使用 `TOP (n)`；若 `n > 1000` 或表述为「全部」「不限制条数」等，仍使用 **`TOP (1000)`**（**主表+明细 JOIN 且精确键全量时除外，见 7a**），并在【相关表】中简短说明「单次最多 1000 条，更多数据需分页或分批查询」。  
...
7a. **主表 + 明细/子表 JOIN（1:N）· TOP 与 Dify 上限**：当 SQL **同时** JOIN 主表与 `*_ITEM` / `*_DTL` 等明细表时：  
   - **必须保留**最外层 `SELECT TOP (1000)`——Dify 出参超过 1000 行会 token 超限报错，**禁止省略 TOP**。  
   - TOP 限制 **JOIN 后最多 1000 行**；用户问「全部/每一项」时，在【相关表】说明「单次最多 1000 行，已按时间优先截断」。  
   - **必须** `ORDER BY` 主表主时间 **DESC** + 主表 **CID DESC** + 明细 **CSEQ ASC**，使 1000 行优先为最新主记录及有序明细项。
