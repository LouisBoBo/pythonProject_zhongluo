# -*- coding: utf-8 -*-
# 【Dify 代码节点 · 读取 SQL 约束规则 · PostgreSQL】
# 维护：改 中络项目MES SQL约束规则提示词.md 后本地执行 seed_mes_sql_rules.py 更新 base 规则
#
# 入参：无
# 出参（string）：rule_list — 仅 learned 增量规则 → SQL LLM【规则约束】（base 约束在系统提示词，勿重复注入）

# -*- coding: utf-8 -*-
"""MES SQL 约束规则 PostgreSQL 读写（基础规则 + 学习规则）。"""
...
_BAD_GLOBAL_207 = re.compile(
    r"^【硬约束·207】字段\s+(CSTATUS|CWC_ID|CSUPPLIER_ID|CITEM_SUPPLIER_ID)\s+不存在",
)
...
_CREATE_TABLE_SQL = """
CREATE TABLE IF NOT EXISTS mes_sql_rules (
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_mes_sql_rules_learned_text
ON mes_sql_rules (rule_text)
...
def should_skip_learned_rule(rule_text: str) -> bool:
    ...
    for line in text.splitlines():
        ...
        if m208 and _normalize_table_name(m208.group(1)) in _MES_CORE_TABLES:
            return True
...
def write_rules_main(
    ...
    **kwargs: Any,
    ...
    log: List[str] = ["=== mes_sql_rules_db_write 开始 ==="]
...
def load_base_rules_from_md(md_path: Optional[Path] = None) -> str:
    ...
    path = Path(md_path) if md_path else root.parent / "中络项目MES SQL约束规则提示词.md"
    if not path.is_file():
