...
def main(error_msg: str = None, **kwargs: Any) -> dict[str, str]:
    ...
    match_207 = re.search(r"Invalid column name '([^']+)'", error_msg)
    if match_207:
    ...
    # === 2. 提取 208 无效表 ===
        ...
        invalid_item = re.search(r"Invalid object name '([^']+)'", error_msg).group(1)
        rules.append(f"【硬约束·208】表 {invalid_item} 不存在，禁止使用。")
        rules.append("【硬约束】表名必须与参考表结构完全一致。")

    # === 3. 提取 102 语法错误（括号 / 斜杠）===
    elif "Incorrect syntax near" in error_msg:
        invalid_item = re.search(r"Incorrect syntax near '([^']+)'", error_msg).group(1)
