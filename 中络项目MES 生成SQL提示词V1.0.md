...
- 若找不到：**删除该列**或换用能找到的列，**禁止**保留猜测列。
- 若该列名与 T-SQL 保留字冲突：在 SQL 中已用 **`[列名]`** 引用（如 `[order]`），而非裸标识符（防 **156**）。
- 若 **`FROM`/`JOIN` 含 `dbo.TBL_SFC_WS_LOG`**：客户编码已用 **`CUSTOMER_CODE`**（**禁止** **`CCUSTOMER_CODE`**）；扫描条码已用 **`CSCAN_BARCODE`**（**禁止** **`CSCANNED_BARCODE`** / **`CSCANN_BARCODE`** 等变体，除非片段逐字另有其列）；若 **`JOIN dbo.TBL_SFC_WS_LOG_ITEM`**：已用 **`l.CID = i.CWS_LOG_ID`**（**禁止** **`l.CWS_LOG_ID`**）。

对每一个 **`AS` 后的中文列别名**（含从字段注释直接复制的「××(××)」「××（××）」）：

- 别名中若含 **`/`、空格、连字符 `-`、半角小括号 `(` `)`、全角括号 `（` `）`** 等：必须 **`AS [别名全文]`**（方括号包住**整段**别名），**禁止** `AS 设备故障子节点(设备ID)` 这类在别名中间裸写半角括号——解析器会把 `(` 当成表达式边界，报 **`Incorrect syntax near '设备ID'`** 等（错误 **102**）。**禁止** **`AS 审核/驳回人`**（裸斜杠），**必须** **`AS [审核/驳回人]`**（错误 **102** near **`/`**）。**正例**：**`r.CWC_CHILDREN AS [设备故障子节点(设备ID)]`**、**`r.CAUDIT_USERNAME AS [审核/驳回人]`**。



对**每一张**用到的表：

- `FROM` / `JOIN` 中的**表名（及视图名）**必须与【参考表结构】里某一节的标题表名**完全一致**（含前缀、下划线）；**禁止**用别名「反推」出一个未在片段中出现的真实表名。
- `SELECT` 列表中的列必须全部来自该表在片段中的字段列表；  
- 若片段中某表字段列表缺失或看不清：对该表**仅使用片段里明确列出的列**，或写 `SELECT … 已列出字段…`；**禁止**用 `CODE`、`NAME`、`CNAME`、`CSTATUS`、`CID` 等泛名补位，除非片段里**真的出现**这些标识符。
- 若用户明确要求「前 N 条」等：**已**在 `SELECT` 后写 `TOP (n)`，且 `n = min(用户要求, 1000)`（用户要求超过 1000 时固定为 1000）。  
- 若用户未要求条数且为可能多行的明细查询：**已**写 `TOP (1000)`（或等价的 `OFFSET … FETCH NEXT 1000 ROWS ONLY`）。  
- 若 `SELECT` 仅含无分组的 `COUNT`/`SUM` 等单值聚合：已**不**在语句末尾对普通列加 `ORDER BY`（防 **8127**）。
...
- 无把握时：两表**分别** `SELECT TOP (1000) *` 或先查 `INFORMATION_SCHEMA` 再写 `JOIN`。
...
【上一次执行的SQL语句】{{#1778469687360.query_sql#}}

【上一次执行的SQL语句报错信息】{{#1778469687360.error_message#}}
