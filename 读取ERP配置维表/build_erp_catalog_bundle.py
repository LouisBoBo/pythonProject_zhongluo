...
# -*- coding: utf-8 -*-
"""
从《中络项目ERP 系统表名清单V1.0.md》打包 Dify 选表清单代码节点：

  python3 build_erp_catalog_bundle.py

维护：
  1. 改 中络项目ERP 系统表名清单V1.0.md
  2. 运行本脚本
  3. 复制 dify_erp_table_catalog.py 到 Dify「输出表名清单」代码节点
"""
...
from erp_table_catalog import count_slim_tables, count_tables, load_catalog_text, slim_catalog_text
from erp_table_catalog_rank import rank_catalog_lines

ROOT = Path(__file__).resolve().parent
MD_FILE = ROOT.parent / "中络项目ERP 系统表名清单V1.0.md"
OUT = ROOT / "dify_erp_table_catalog.py"
RANK_CORE = (ROOT / "erp_table_catalog_rank.py").read_text(encoding="utf-8")
...
def main() -> None:
    ...
    for q in ("查制成品接收明细", "制造订单列表", "采购订单明细", "工单过数记录"):
        ranked, n, mode = rank_catalog_lines(q, slim, top_n=40)
