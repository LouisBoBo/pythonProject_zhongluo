...
# -*- coding: utf-8 -*-
"""
解析 erp_dimension_joins.map（表格式文本）→ 与 erp_dimension_joins.json 相同结构的 dict。
...
DEFAULT_MAP = Path(__file__).resolve().parent / "erp_dimension_joins.map"
DEFAULT_JSON = Path(__file__).resolve().parent / "erp_dimension_joins.json"
...
def parse_dimension_map(text: str) -> Dict[str, Any]:
    config: Dict[str, Any] = {
        "version": 1,
        "description": "由 erp_dimension_joins.map 解析生成",
